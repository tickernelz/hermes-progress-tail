...
def test_focused_footer_shows_github_latest_release_update_only_when_newer(monkeypatch):
    async def run():
        def update_info():
            return FooterInfo("0.2.11", "v0.2.12", "https://example.test/v0.2.12")
        ...
        assert "**__Status__**" in content
        assert "⬆️ update `v0.2.12`" in content
        assert "https://example.test/v0.2.12" in content
        assert content.rfind("**__Status__**") > content.rfind("**__Tools__**")

        adapter2 = EditableAdapter()
        renderer2 = make_renderer()
        renderer2.footer_info_provider = lambda: FooterInfo("0.2.11", "v0.2.11", "")
        ...
        assert "v0.2.11" not in current_content
