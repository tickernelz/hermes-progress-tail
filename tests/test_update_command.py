...
def test_update_command_dry_run_uses_latest_release_without_writing(monkeypatch):
    ...
    assert calls == [{"ref": "v0.1.95", "dry_run": True, "profiles": None, "all_profiles": False}]
    assert "Update dry-run: v0.1.94 → v0.1.95" in output
...
def test_update_command_apply_can_use_explicit_ref(monkeypatch):
    ...
    output = plugin._command("update --apply --ref v0.1.95 --profile default")

    assert calls == [("v0.1.95", False, ["default"], False)]
    assert "Update applied: v0.1.94 → v0.1.95" in output
...
def test_update_command_skips_when_latest_is_not_newer(monkeypatch):
    ...
    assert called is False
    assert "Already up to date: v0.1.94" in output
...
def test_update_command_alias_defaults_to_apply(monkeypatch):
    ...
    assert calls == [("v0.1.95", False, None, False)]
