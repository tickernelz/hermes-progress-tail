...
def test_update_command_dry_run_uses_latest_release_without_writing(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: {"tag_name": "v0.2.08", "html_url": "https://example.test/v0.2.08"},
    )
    ...
    assert calls == [{"ref": "v0.2.08", "dry_run": True, "profiles": None, "all_profiles": False}]
    assert "Update dry-run: v0.2.07 → v0.2.08" in output
...
def test_update_command_apply_can_use_explicit_ref(monkeypatch):
    ...
    output = plugin._command("update --apply --ref v0.2.08 --profile default")

    assert calls == [("v0.2.08", False, ["default"], False)]
    assert "Update applied: v0.2.07 → v0.2.08" in output
...
def test_update_command_skips_when_latest_is_not_newer(monkeypatch):
    ...
    monkeypatch.setattr(
        commands,
        "_latest_release_info",
        lambda: {"tag_name": "v0.2.07", "html_url": "https://example.test/v0.2.07"},
    )
    ...
    assert called is False
    assert "Already up to date: v0.2.07" in output
...
def test_update_command_alias_defaults_to_apply(monkeypatch):
    ...
    assert calls == [("v0.2.08", False, None, False)]
