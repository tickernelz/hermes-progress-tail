...
def test_update_command_dry_run_uses_latest_release_without_writing(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: {"tag_name": "v0.2.03", "html_url": "https://example.test/v0.2.03"},
    )
    ...
    assert calls == [{"ref": "v0.2.03", "dry_run": True, "profiles": None, "all_profiles": False}]
    assert "Update dry-run: v0.2.02 → v0.2.03" in output
...
def test_update_command_apply_can_use_explicit_ref(monkeypatch):
    ...
    output = plugin._command("update --apply --ref v0.2.03 --profile default")

    assert calls == [("v0.2.03", False, ["default"], False)]
    assert "Update applied: v0.2.02 → v0.2.03" in output
...
def test_update_command_skips_when_latest_is_not_newer(monkeypatch):
    ...
    monkeypatch.setattr(
        commands,
        "_latest_release_info",
        lambda: {"tag_name": "v0.2.02", "html_url": "https://example.test/v0.2.02"},
    )
    ...
    assert called is False
    assert "Already up to date: v0.2.02" in output
...
def test_update_command_alias_defaults_to_apply(monkeypatch):
    ...
    assert calls == [("v0.2.03", False, None, False)]
