...
def _command(raw_args: str = "") -> str:
    ...
    if args in {"", "status", "doctor"}:
        ...
        lines = [
            ...
            f"telegram=rich:{settings.telegram.rich_messages} table:{settings.telegram.verification_table} thinking:{settings.telegram.thinking_blocks} max_table_rows:{settings.telegram.max_table_rows} compact_success:{settings.telegram.compact_success} max_detail_items:{settings.telegram.max_detail_items} collapse:{settings.telegram.collapse_narrative_chars}",
